...
class CairoUpdater():
    ...
    def update_speed(self, robot_x, robot_y):
        ...
        if time.time() > self.speed_last_update_time + ui_update_rate:
            ...
            self.cairo_overlay_obj.speed_text = round(speed, 2)
    ...
    def update_stanley_reset(self, to_stanley_reset):
        if to_stanley_reset == 1:
            ...
            self.cairo_overlay_obj.stanley_path_reset_state = 1
        elif time.time() > self.to_reset_press_time + 2:
            self.cairo_overlay_obj.stanley_path_reset_state = 0

    def update_speed_limiter(self, to_speed_limiter):
        self.cairo_overlay_obj.speed_limiter_text = to_speed_limiter

    def update_gps_status(self, robot_x, robot_y):
        self.cairo_overlay_obj.gps_status_text = f"X:{robot_x}, Y:{robot_y}"

    def update_stanley_params(self, k, v):
        self.cairo_overlay_obj.stanley_k_text = k
        self.cairo_overlay_obj.stanley_v_text = v

    def update_pid_values(self, final_speed, kp, ki, kd):
        self.cairo_overlay_obj.pid_values = [final_speed, kp, ki, kd]

    def update_gps_saving_toggle(self, gps_saving_toggle):
        if gps_saving_toggle == 1:
            self.cairo_overlay_obj.gps_path_saving_text = "SAVING PATH"
        else:
            self.cairo_overlay_obj.gps_path_saving_text = "Not Saving"

    def update_final_point_status(self, final_point_status):
        if final_point_status == 1:
            self.cairo_overlay_obj.stanley_at_final_point_text = "Path Complete"
        else:
            self.cairo_overlay_obj.stanley_at_final_point_text = "Not Complete"

    def update_stanley_running_status(self, stanley_drive_toggle):
        if stanley_drive_toggle == 1:
            self.cairo_overlay_obj.stanley_running_text = "Running"
        else:
            self.cairo_overlay_obj.stanley_running_text = "Not Running"

    def update_gear(self, to_handbrake_toggle, to_reverse_toggle):
        if to_handbrake_toggle == 1:
            self.cairo_overlay_obj.current_gear_text = "HB"
        elif to_reverse_toggle == 1:
            self.cairo_overlay_obj.current_gear_text = "R"
        else:
            self.cairo_overlay_obj.current_gear_text = "D"
